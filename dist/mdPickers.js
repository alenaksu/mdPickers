...
function TimePickerCtrl($scope, $mdDialog, time, autoSwitch, $mdMedia, dialogAccent) {
    var self = this;
    this.VIEW_HOURS = 1;
    this.VIEW_MINUTES = 2;
    ...
    this.dialogAccent = dialogAccent;
    ...
    return {
        ...
        template:   '<div class="mdp-clock">' +
                        '<div class="mdp-clock-container">' +
                            ...
                            '</md-toolbar>' +
                            '<md-button ng-class="{ \'md-primary\': clock.selected == step, \'md-accent\': clock.selected == step && clock.dialogAccent }" class="md-icon-button md-raised mdp-clock-deg{{ ::(clock.STEP_DEG * ($index + 1)) }}" ng-repeat="step in clock.steps" ng-click="clock.setTime(step)">{{ step }}</md-button>' +
                        '</div>' +
                    '</div>',
    ...
    this.$get = ["$mdDialog", function ($mdDialog) {
        var timePicker = function (time, options) {
            ...
            return $mdDialog.show({
                controller: ['$scope', '$mdDialog', 'time', 'autoSwitch', '$mdMedia', 'dialogAccent', TimePickerCtrl],
                controllerAs: 'timepicker',
                clickOutsideToClose: true,
                template:   '<md-dialog aria-label="" class="mdp-timepicker" ng-class="{ \'portrait\': !$mdMedia(\'gt-xs\') }">' +
                                '<md-dialog-content layout-gt-xs="row" layout-wrap>' +
                                '<md-toolbar layout-gt-xs="column" layout-xs="row" layout-align="center center" flex class="mdp-timepicker-time md-hue-1 md-primary" ng-class="{\'md-accent\': timepicker.dialogAccent}">' +
                                ...
                                '<div>' +
                                    '<div class="mdp-clock-switch-container" ng-switch="timepicker.currentView" layout layout-align="center center">' +
                                        '<mdp-clock class="mdp-animation-zoom" auto-switch="timepicker.autoSwitch" time="timepicker.time" type="hours" ng-switch-when="1" dialog-accent="timepicker.dialogAccent"></mdp-clock>' +
                                        '<mdp-clock class="mdp-animation-zoom" auto-switch="timepicker.autoSwitch" time="timepicker.time" type="minutes" ng-switch-when="2" dialog-accent="timepicker.dialogAccent"></mdp-clock>' +
                                    '</div>' +
                ...
                locals: {
                    ...
                    dialogAccent: options.dialogAccent
                },
                skipHide: true
            });
        };
...
module.directive("mdpTimePicker", ["$mdpTimePicker", "$timeout", function ($mdpTimePicker, $timeout) {
    return {
        ...
        link: function (scope, element, attrs, ngModel, $transclude) {
            ...
            scope.dialogAccent = scope.dialogAccent || false;
            ...
            scope.showPicker = function (ev) {
                $mdpTimePicker(ngModel.$modelValue, {
                    ...
                    dialogAccent: scope.dialogAccent
                }).then(function (time) {
...
function DatePickerCtrl($scope, $mdDialog, $mdMedia, $timeout, currentDate, options, dialogAccent) {
    var self = this;
...
module.provider("$mdpDatePicker", function () {
    ...
    this.$get = ["$mdDialog", function ($mdDialog) {
        var datePicker = function (currentDate, options) {
            ...
            return $mdDialog.show({
                controller: ['$scope', '$mdDialog', '$mdMedia', '$timeout', 'currentDate', 'options', 'dialogAccent', DatePickerCtrl],
                controllerAs: 'datepicker',
                clickOutsideToClose: true,
                template:   '<md-dialog aria-label="" class="mdp-datepicker" ng-class="{ \'portrait\': !$mdMedia(\'gt-xs\') }">' +
                                '<md-dialog-content layout="row" layout-wrap>' +
                                    '<div layout="column" layout-align="start center">' +
                                        '<md-toolbar layout-align="start start" flex class="mdp-datepicker-date-wrapper md-hue-1 md-primary" layout="column" ng-class="{\'md-accent\': datepicker.dialogAccent}">' +
                                        ...
                                        '<div class="mdp-datepicker-select-year mdp-animation-zoom" layout="column" layout-align="center start" ng-if="datepicker.selectingYear">' +
                                            '<md-virtual-repeat-container md-auto-shrink md-top-index="datepicker.yearTopIndex">' +
                                                '<div flex md-virtual-repeat="item in datepicker.yearItems" md-on-demand class="repeated-year">' +
                                                    '<span class="md-button" ng-click="datepicker.selectYear(item)" md-ink-ripple ng-class="{ \'md-primary current\': item == year, \'md-accent\': item == year && datepicker.dialogAccent }">{{ item }}</span>' +
                                                '</div>' +
